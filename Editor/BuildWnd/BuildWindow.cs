...
namespace AssetBundle
{
    public class BuildWindow : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            BuildBundlePrefs.SelectedBuildPlaceIndex = GUILayout.SelectionGrid(BuildBundlePrefs.SelectedBuildPlaceIndex, buildBundlePlaceArray, buildBundlePlaceArray.Length);
            GUILayout.EndHorizontal();
            GUILayout.BeginHorizontal();
            GUILayout.Label("", GUILayout.Width(150));
            BuildBundlePrefs.BuildBundleFolderPath = GUILayout.TextField(BuildBundlePrefs.BuildBundleFolderPath);
            if (BuildBundlePrefs.SelectedBuildPlaceIndex == 0)
                BuildBundlePrefs.BuildBundleFolderPath = Application.streamingAssetsPath;
            else if (BuildBundlePrefs.SelectedBuildPlaceIndex == 1)
                BuildBundlePrefs.BuildBundleFolderPath = Path.GetFullPath($"{Application.dataPath}/../../AssetBundleDB/{Args.CurBuildTarget.ToString()}");
            else if (BuildBundlePrefs.SelectedBuildPlaceIndex == 2)
            {
                if (GUILayout.Button("修改", GUILayout.Width(50)))
                {
                    BuildBundlePrefs.BuildBundleFolderPath = EditorUtility.SaveFolderPanel("选择导出AssetBundle目录", BuildBundlePrefs.BuildBundleFolderPath, BuildBundlePrefs.BuildBundleFolderPath);
                }
            }
            GUILayout.EndHorizontal();
            GUILayout.Space(10);
            ...
            Args.CurBuildTarget = (BuildTargetEnum)EditorGUILayout.EnumPopup(Args.CurBuildTarget);
            GUILayout.EndHorizontal();
            GUILayout.Space(30);
        ...
        public IEnumerator BuildExcute(BuildArguments args, bool showDialog)
        {
            ...
            {
                BuildTarget target = (BuildTarget)Args.CurBuildTarget;
            ...
            // 创建打包的文件夹
            if (!Directory.Exists(BuildBundlePrefs.BuildBundleFolderPath))
                Directory.CreateDirectory(BuildBundlePrefs.BuildBundleFolderPath);
